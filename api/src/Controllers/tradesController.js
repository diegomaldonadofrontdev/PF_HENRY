...
const searchTradeByName = async (name) => {
	// OK
	try {
		let tradeFound = await Trade.find();
		let tradeCoincidence = tradeFound.filter((t) =>
			t.commerceName.toLowerCase().includes(name.toLowerCase())
		);
...
const resetPasswordController = async (password, token) => {
	...
	try {
		...
		if (!trade)
			return `No hemos encontrado un comercio con ese correo electrónico.`;
...
const createDeliveryZone = async (deliZone) => {
	// OK
	try {
...
const deleteCaegory = async (category) => {
	try {
		...
		if (deleted.deletedCount !== 0)
			return `Se eliminó la categoría ${category}`;
...
const deleteSubcategory = async (subcategory) => {
	try {
		...
		if (deleted.deletedCount !== 0)
			return `Se eliminó la subcategoría ${subcategory}`;
		return `No fue posible eliminar la subcategoría ${subcategory}`;
	} catch (error) {
		throw new Error(
			`Ocurrió un error al eliminar la subcategoría ${subcategory}`
		);
	}
};
